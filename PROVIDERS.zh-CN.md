...
通过 [SimpleLogin](https://simplelogin.io) 管理邮箱别名。
...
| 参数 | 类型 | 必填 | 说明 |
|---|---|---|---|
| `type` | `'simple-login'` | 是 | 提供商标识符 |
| `apiKey` | `string` | 是 | SimpleLogin API 密钥 |
| `apiUrl` | `string` | 否 | 自建实例的 API URL（默认: `https://app.simplelogin.io`） |

#### 获取 API Key

1. 登录 [app.simplelogin.io](https://app.simplelogin.io)
2. 进入账户设置中的 **API Keys** 页面
3. 点击 **创建新 API 密钥**
4. 复制生成的 API 密钥 → 用作 `apiKey`

#### 自建实例

如果使用自建 SimpleLogin 实例，请将 `apiUrl` 设置为服务器的基础 URL（例如 `https://sl.example.com`）。
